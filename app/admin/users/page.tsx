...
const roleStyles: Record<string, string> = {
  USER: "bg-slate-900 text-white border-slate-900",
  DOCTOR: "bg-emerald-100 text-emerald-700 border-emerald-200",
  ADMIN: "bg-rose-100 text-rose-700 border-rose-200",
  "DOCTOR MANAGER": "bg-indigo-100 text-indigo-700 border-indigo-200",
  MODERATOR: "bg-purple-100 text-purple-700 border-purple-200",
  MANAGER: "bg-fuchsia-100 text-fuchsia-700 border-fuchsia-200",
};
...
export default function AdminUserManagementPage() {
  ...
  const handleConfirmDeleteUser = async () => {
    ...
    const toastId = toast.loading(language === "km" ? "កំពុងលុបគណនី..." : "Deleting user...");
    try {
      await apiClient.patch(`/api/v1/admin/users/${deleteDialogUser.id}/soft-delete`);
      toast.dismiss(toastId);
      toast.success(language === "km" ? "គណនីត្រូវបានលុបដោយជោគជ័យ" : "User successfully deleted");
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Badge
                          className={`rounded px-2.5 py-0.5 border text-[10px] font-extrabold select-none tracking-wider ${
                            roleStyles[user.role.toUpperCase()] || "bg-slate-100 text-slate-700"
                          }`}
                        >
                        ...
                        <div className="relative flex items-center justify-end gap-1.5">
                          ...
                          {activeMenuId === user.id && (
                            <div className="absolute right-0 top-9 z-50 w-44 rounded-xl border border-slate-100 bg-white py-1 shadow-lg ring-1 ring-black/5 animate-in fade-in slide-in-from-top-1">
                              ...
                              >
                                {user.status === "Active" ? t("users.suspendUser") : t("users.activateUser")}
                              </button>
                              <button
                                onClick={() => handleDeleteUser(user)}
                                className="flex w-full items-center px-4 py-2 text-left text-xs font-bold text-red-600 hover:bg-red-50"
                              >
                                {t("users.deleteUser")}
                              </button>
                            </div>
        ...
        <DialogContent className="sm:max-w-[425px] bg-white border-slate-200 rounded-2xl shadow-xl">
          <DialogHeader className="gap-2">
            ...
            <DialogTitle className="text-xl font-bold text-center tracking-tight text-slate-900">
              {language === "km" ? "បញ្ជាក់ការលុបគណនី" : "Confirm Account Deletion"}
            </DialogTitle>
            <DialogDescription className="text-center text-slate-500 text-sm leading-relaxed pt-2">
              {language === "km" ? (
                <>
                  តើអ្នកពិតជាចង់លុបគណនី <span className="font-bold text-slate-900">"{deleteDialogUser?.name}"</span> មែនទេ? 
                  <br/><br/>
                  សកម្មភាពនេះនឹងលុប និងលាក់អត្តសញ្ញាណ (Anonymize) របស់គណនីនេះជាអចិន្ត្រៃយ៍ ដើម្បីរក្សាសុវត្ថិភាពទិន្នន័យ (Data Integrity) ដោយមិនប៉ះពាល់ដល់ប្រវត្តិឯកសារពេទ្យ។
                </>
              ) : (
                <>
                  Are you sure you want to permanently delete <span className="font-bold text-slate-900">"{deleteDialogUser?.name}"</span>? 
                  <br/><br/>
                  This action will permanently anonymize their personal information to maintain clinical data integrity without breaking historical records.
                </>
              )}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-col gap-3 mt-4 pt-4 border-t border-slate-100">
            ...
            >
              {t("user.modal.cancel" as any) || "Cancel"}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
