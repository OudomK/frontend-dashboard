    ...
    >
      <div className="space-y-5 pb-24 lg:pb-6">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 rounded-2xl border border-dashed border-slate-200 bg-slate-50 text-center">
            <Newspaper className="h-12 w-12 text-slate-200 mb-3" />
            <p className="font-bold text-slate-500 mb-1">No articles found</p>
            <p className="text-sm text-slate-400">Try adjusting your filters or create a new article.</p>
            <Button
              onClick={() => { resetForm(); setOpenCreateDialog(true); }}
              className="mt-4 h-9 gap-1.5 rounded-xl bg-blue-600 px-4 text-sm font-semibold text-white hover:bg-blue-700"
            >
      ...
      <Dialog open={openCreateDialog} onOpenChange={(open) => { if (!open) resetForm(); setOpenCreateDialog(open); }}>
        <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-3xl rounded-2xl shadow-2xl p-0">
          {/* Header */}
          ...
          <div className="px-6 py-5 space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full rounded-xl border border-slate-200 p-4 text-sm outline-none transition-all placeholder:text-slate-300 focus:border-blue-400 focus:ring-2 focus:ring-blue-100 resize-y"
              />
      ...
      <Dialog open={openPreviewDialog} onOpenChange={setOpenPreviewDialog}>
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl rounded-2xl shadow-2xl p-0">
